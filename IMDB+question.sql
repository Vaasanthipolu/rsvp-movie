...
SELECT 
    (SELECT 
            COUNT(*)
        FROM
            director_mapping) AS director_mapping_rows,
    (SELECT 
            COUNT(*)
        FROM
            genre) AS genre_rows,
    ...
    (SELECT 
            COUNT(*)
        FROM
            ratings) AS ratings_rows,
    (SELECT 
            COUNT(*)
        FROM
            role_mapping) AS role_mapping_rows;
...
SELECT 
    (SELECT 
        ...
        FROM
            movie
        WHERE
            id IS NULL) AS idnull,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            title IS NULL) AS titlenull,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            year IS NULL) AS yearnull,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            date_published IS NULL) AS date_null,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            duration IS NULL) AS durationnull,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            country IS NULL) AS countrynull,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            worlwide_gross_income IS NULL) grossnull,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            languages IS NULL) langnull,
    (SELECT 
            COUNT(*)
        FROM
            movie
        WHERE
            production_company IS NULL) procompnull;
...
FROM
    movie
WHERE
    year = '2019' AND country LIKE 'USA'
...
FROM
    ...
    FROM
        genre
    GROUP BY movie_id
    HAVING genre_count = 1) AS one_genre;
...
-- Type your code below:
    SELECT 
    total_votes AS votes, languages
FROM
    ratings AS r
        JOIN
...
WHERE 
  r.avg_rating > 8 
  AND g.genre IN (cte_top_genres.genre) 
GROUP BY 
  n.name 
ORDER BY 
  movie_count DESC 
...
FROM
  ...
  JOIN ratings AS r ON r.movie_id = rm.movie_id
...
cte_top_gross AS(
  SELECT
    ...
    year,
    title AS movie_name,
   REPLACE( REPLACE(worlwide_gross_income, "INR ",""), "$ ","") AS worlwide_gross_income ,
...
-- Type your code below:
 SELECT
  ...
  Count(r.movie_id) AS movie_count,
  Round(
    Avg((avg_rating * total_votes)/ total_votes),2)
 AS actress_avg_rating,
  Dense_rank() over(
    ORDER BY
...
--------------------------------------------------------------------------------------------*/
-- Type you code below:
with cte_avg_inter_movie_days AS(
  SELECT 
...
SELECT 
  ...
  round(
   ...
   round(avg((avg_rating*total_votes)/total_votes),2)   AS avg_rating, 
  sum(total_votes) AS total_votes, 
  min(avg_rating) AS min_rating, 
  max(avg_rating) AS max_rating, 
  sum(duration) AS total_duration 
FROM 
  cte_date_diff AS cte 
GROUP BY 
  director_id 
ORDER BY 
  count(movie_id) DESC, 
